    ...
    while True:
        try:
            ...
            if message_photo is not None and message_photo['data']:
                ...
                predicted['categories'] = predicted['categories'].sorted(key=lambda x: x['prob'], reverse=True)[:10]
                predicted['objects'] = predicted['objects'].sorted(key=lambda x: x['prob'], reverse=True)
